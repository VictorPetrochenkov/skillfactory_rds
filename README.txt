# skillfactory_rds
Файл  Unit_1  содержит код анализа датасет в котором приведены 1889 строк параметров фильмов вышедших в кинопрокатв период с  2000 по 2015 годы.
Вопрос 1. У какого фильма из списка самый большой бюджет?
Ответ на вопрос № 1 723    Pirates of the Caribbean: On Stranger Tides
Name: original_title, dtype: object
Ответ на вопрос № 1 723    tt1298650
Name: imdb_id, dtype: object
----------------
Вопрос 2. Какой из фильмов самый длительный (в минутах)?
Ответ на вопрос № 2 1157    Gods and Generals
Name: original_title, dtype: object
Ответ на вопрос № 2 1157    tt0279111
Name: imdb_id, dtype: object
----------------
Вопрос 3. Какой из фильмов самый короткий (в минутах)?
Ответ на вопрос № 3 768    Winnie the Pooh
Name: original_title, dtype: object
Ответ на вопрос № 3 768    tt1449283
Name: imdb_id, dtype: object
----------------
Вопрос 4. Какова средняя длительность фильмов?
Ответ на вопрос № 4 110.0
----------------
Вопрос 5. Каково медианное значение длительности фильмов?
Ответ на вопрос № 5 107.0
----------------
Вопрос 6. Какой фильм самый прибыльный?
Ответ на вопрос № 6 239    Avatar
Name: original_title, dtype: object
Ответ на вопрос № 6 239    tt0499549
Name: imdb_id, dtype: object
----------------

7. Какой фильм самый убыточный?
Ответ на вопрос № 7 1245    The Lone Ranger
Name: original_title, dtype: object
Ответ на вопрос № 7 1245    tt1210819
Name: imdb_id, dtype: object
----------------
Вопрос 8. У скольких фильмов из датасета объем сборов оказался выше бюджета?
Ответ на вопрос № 8 1478
----------------
Вопрос 9. Какой фильм оказался самым кассовым в 2008 году?
Ответ на вопрос № 9 599    The Dark Knight
Name: original_title, dtype: object
Ответ на вопрос № 9 599    tt0468569
Name: imdb_id, dtype: object
----------------
Вопрос 10. Самый убыточный фильм за период с 2012 по 2014 годы (включительно)?
Ответ на вопрос № 10 1245    The Lone Ranger
Name: original_title, dtype: object
Ответ на вопрос № 10 1245    tt1210819
Name: imdb_id, dtype: object
----------------
Вопрос 11. Какого жанра фильмов больше всего?
Ответ на вопрос № 11 Drama
----------------
Вопрос 12. Фильмы какого жанра чаще всего становятся прибыльными?
Ответ на вопрос № 12 Drama
----------------
Вопрос 13. У какого режиссера самые большие суммарные кассовые сбооры?
Ответ на вопрос № 13 Peter Jackson
----------------
Вопрос 14. Какой режиссер снял больше всего фильмов в стиле Action?
Ответ на вопрос № 14 Robert Rodriguez
----------------
Вопрос 15. Фильмы с каким актером принесли самые высокие кассовые сборы в 2012 году?
Ответ на вопрос № 15 Chris Hemsworth
----------------
Вопрос 16. Какой актёр снялся в большем количестве высокобюджетных фильмов?
Ответ на вопрос № 16 Matt Damon
----------------
Вопрос 17. В фильмах какого жанра больше всего снимался Nicolas Cage?
Ответ на вопрос № 17 Action
----------------

Вопрос 18. Самый убыточный фильм от Paramount Pictures?
Ответ на вопрос № 18 925    K-19: The Widowmaker
Name: original_title, dtype: object
Ответ на вопрос № 18 925    tt0267626
Name: imdb_id, dtype: object
----------------
Вопрос 19. Какой год стал самым успешным по суммарным кассовым сборам?
Ответ на вопрос № 19 2015
----------------
Вопрос 20. Какой самый прибыльный год для студии Warner Bros?
Ответ на вопрос № 20 2014
----------------
Вопрос 21. В каком месяце за все годы суммарно вышло больше всего фильмов?
Ответ на вопрос № 21 Сентябрь
----------------
Вопрос 22. Сколько суммарно вышло фильмов летом? (за июнь, июль, август)
Ответ на вопрос № 22 450
----------------
Вопрос 23. Для какого режиссера зима — самое продуктивное время года?
Ответ на вопрос № 23 Peter Jackson
----------------
Вопрос 24. Какая студия даёт самые длинные названия своим фильмам по количеству символов?
D:\Program Files\Python_3\lib\site-packages\pandas\core\strings.py:1954: UserWarning: This pattern has match groups. To actually get the groups, use str.extract.
  return func(self, *args, **kwargs)
Ответ на вопрос № 24 Four By Two Productions
----------------
Вопрос 25. Описание фильмов какой студии в среднем самые длинные по количеству слов?
Ответ на вопрос № 25 Midnight Picture Show
----------------
Вопрос 26. Какие фильмы входят в один процент лучших по рейтингу?
Ответ на вопрос № 26 [('The Dark Knight', 8.1), ('The Imitation Game', 8.0), ('Interstellar', 8.0), ('Room', 8.0), ('Inside Out', 8.0), ('Memento', 7.9), ('The Return', 7.9), ('12 Years a Slave', 7.9), ('The Wolf of Wall Street', 7.9), ('The Lord of the Rings: The Return of the King', 7.9), ('The Pianist', 7.9), ('Inception', 7.9), ('The Grand Budapest Hotel', 7.9), ('Gone Girl', 7.9), ('Guardians of the Galaxy', 7.9), ('There Will Be Blood', 7.8), ('Eternal Sunshine of the Spotless Mind', 7.8), ('The Prestige', 7.8), ('Dallas Buyers Club', 7.8)]
----------------
Вопрос 27. Какие актеры чаще всего снимаются в одном фильме вместе?
2 = Robert Downey Jr. & Chris Evans
3 = Robert Downey Jr. & Mark Ruffalo
3 = Robert Downey Jr. & Scarlett Johansson
4 = Chris Evans & Scarlett Johansson
4 = Scarlett Johansson & Chris Evans
4 = Christian Bale & Michael Caine
4 = Michael Caine & Christian Bale
5 = Ian McKellen & Hugh Jackman
5 = Kristen Stewart & Robert Pattinson
5 = Kristen Stewart & Taylor Lautner
6 = Johnny Depp & Helena Bonham Carter
6 = Helena Bonham Carter & Johnny Depp
8 = Emma Watson & Daniel Radcliffe
8 = Daniel Radcliffe & Emma Watson
----------------
ОТВЕТЫ НА ВОПРОСЫ
1 Pirates of the Caribbean: On Stranger Tides (tt1298650)
2 Gods and Generals (tt0279111)
3 Winnie the Pooh (tt1449283)
4 110
5 107
6 Avatar (tt1449283)
7 The Lone Ranger (tt1210819)
8 1478
9 The Dark Knight (tt0468569)
10 The Lone Ranger (tt1210819)
11 Drama
12 Drama
13 Peter Jackson
14 Robert Rodriguez
15 Chris Hemsworth
16 Matt Damon
17 Action
18 K-19: The Widowmaker (tt0267626)
19 2015
20 2014
21 Сентябрь
22 450
23 Peter Jackson
24 Four By Two Productions
25 Midnight Picture Show
26 Inside Out, The Dark Knight, 12 Years a Slave
27 Daniel Radcliffe & Rupert Grint
Количество ответов 27
